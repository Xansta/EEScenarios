...
-- Setting[Weather]: Configures whether asteroids and/or nebulae move across the play area.
-- Weather[All|Default]: Both asteroids and nebulae will move across the play area.
-- Weather[Asteroids]: Just asteroids will move across the play area.
-- Weather[Nebulae]: Just nebulae will move across the play area.
-- Weather[None]: No asteroids or nebulae will move across the play area. Clear skies!
require("utils.lua")
...
function init()
	stats = {}
	enemy_chasers = {}
	chaser_delay_time = 600	--10 is for test, final: 10*60 = 600 (10 minutes)
	...
	local weather_config = {
		["All"] =		{asteroids = true,	nebulae = true},
		["Asteroids"] =	{asteroids = true,	nebulae = false},
		["Nebulae"] =	{asteroids = false,	nebulae = true},
		["None"] =		{asteroids = false,	nebulae = false},
	}
	asteroid_weather = weather_config[getScenarioSetting("Weather")].asteroids
	nebula_weather = weather_config[getScenarioSetting("Weather")].nebulae
end
...
function constructEnvironment()
	...
	enemy_station_x, enemy_station_y = vectorFromAngle(enemy_station_angle,random(50000,80000),true)
	...
	for i=1,10 do
		...
		transport:orderDock(tableSelectRandom(stations)):setCommsScript("comms_ship.lua")
...
function commsFlagship()
	...
	addCommsReply(_("goal-comms","What is our mission?"),function()
		local primary_mission = _("goal-comms","Your mission is to destroy the Kraylor station.")
		if getScenarioSetting("Level") ~= "Easy" then
			primary_mission = _("goal-comms","Your mission is to destroy the Kraylor station and ships.")
		end
		if comms_source.distressed_freighter_message ~= nil then
			setCommsMessage(string.format(_("goal-comms","%s You have also been asked to help a distressed Arlenian freighter."),primary_mission))
		else
			setCommsMessage(primary_mission)
		end
		addCommsReply(_("Back"),commsFlagship)
	end)
	...
	if enemy_station:isValid() then
		...
		if clean_list and #enemy_chasers < 1 then
			if chase_time == nil then
				chase_time = getScenarioTime() + chaser_delay_time + random(1,60)	--3 is for test, final: 60 
			end
			if getScenarioTime() > chase_time then
				chaser_delay_time = 300	--5 is for test, final: 60*5 = 300 (5 minutes)
				chase_time = nil
...
function makeWeatherNebula()
	local n = Nebula()
	local origin_x, origin_y = vectorFromAngle(weather_angle + 90,random(-100000,100000),true)
	n.origin_x = origin_x + base_origin_x
	n.origin_y = origin_y + base_origin_y
	local nx, ny = vectorFromAngle(weather_angle,random(0,200000),true)
	n:setPosition(n.origin_x + nx, n.origin_y + ny)
	n.speed = random(5,20)
	return n
end
function maintainWeather()
	if asteroid_weather or nebula_weather then
		if weather_angle == nil then
			...
			if asteroid_weather then
				moving_asteroids = {}
				for i=1,100 do
					table.insert(moving_asteroids,makeWeatherAsteroid(false))
				end
			end
			if nebula_weather then
				moving_nebulae = {}
				for i=1,20 do
					table.insert(moving_nebulae,makeWeatherNebula())
				end
			end
		else
			if asteroid_weather then
			...
			if nebula_weather then
				for i,n in ipairs(moving_nebulae) do
					local nx, ny = n:getPosition()
					if distance(nx, ny, n.origin_x, n.origin_y) > 200000 then
						n:setPosition(n.origin_x, n.origin_y)
					else
						local dx, dy = vectorFromAngle(weather_angle,n.speed,true)
						n:setPosition(nx + dx, ny + dy)
					end
				end
			end
		end
...
function distressedFreighter()
	if distressed_freighter_time == nil then
		distressed_freighter_time = getScenarioTime() + 300	--5 is test, final: 300 (5 minutes)
	...
	if getScenarioTime() > distressed_freighter_time then
		if distressed_freighter_dies_time == nil then
			distressed_freighter_dies_time = getScenarioTime() + 180	--5 is for test, final is 2 - 4 minutes
		end
		for i,p in ipairs(getActivePlayerShips()) do
...
function removeFreighterWidgets()
	for i,p in ipairs(getActivePlayerShips()) do
		...
		p:removeCustom(p.freighter_time_info_ops)
	end
end
