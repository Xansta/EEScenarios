...
function constructEnvironment()
	...
	distressed_freighter_x, distressed_freighter_y = vectorFromAngle(enemy_station_angle + random(120,240),random(50000,80000),true)
...
function commsFlagship()
	...
	if comms_source.distressed_freighter_message ~= nil then
		addCommsReply("Where is the distressed Arlenian freighter?",function()
			...
			setCommsMessage(string.format("The signal from the distressed Arlenian freighter came to %s on bearing %s. You are currently on bearing %s from %s.",home_station:getCallSign(),bearing_to_freighter,bearing_to_player,home_station:getCallSign()))
		end)
	end
end
...
function bugThePlayer()
	...
	if clean_list and #enemy_ships < 1 then
		if bug_time == nil then
			bug_time = getScenarioTime() + 3	--3 is for test, final: 120 (2 minutes)
		end
	...
	if getScenarioTime() > distressed_freighter_time then
		for i,p in ipairs(getActivePlayerShips()) do
			if p.distressed_freighter_message == nil then
				local bearing_to_freighter = math.floor(angleHeading(home_station,distressed_freighter))
				p:addToShipLog(string.format("Faint distress call from Arlenian freighter detected on bearing %s. Please investigate and help if you can.",bearing_to_freighter),"Magenta")
				p.distressed_freighter_message = "sent"
			end
			if p.render_aid_button_rel == nil then
				p.render_aid_button_rel = "render_aid_button_rel"
				p:addCustomButton("Relay",p.render_aid_button_rel,"Render Aid",function()
					string.format("")
					renderAid(p)
				end)
			end
...
function renderAid(p)
	string.format("")
	if distance(p,distressed_freighter) < 5000 then
		p:removeCustom(p.render_aid_button_rel)
		p:addToShipLog(string.format("You have successfully rendered aid to %s, the distressed Arlenian freighter. They are grateful. You receive 100 reputation points.",distressed_freighter:getCallSign()),"Magenta")
